...
//Create and add a layer on the map the presents the gNodeB devices available.
...
function getCores()
{
    const url = 'http://150.140.195.214:3000/devices/getCores';
    fetch(url,)
    .then((resp) => resp.json())
    .then(function(resp) {
      my_cores=resp
...
function toggleOnOff(e) 
{
  ...
  //Start or stop the radio as requested
  config(my_devID)
  console.log(my_devID)
  //Change the icon from green to red and vice verca depending on state/action
...
function handleTab(evt, category) {
  ...
  //Once a ctegory is selected the mpa is initialised and the requested layers added
  handleMap(category)
}
